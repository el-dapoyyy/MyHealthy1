...
public class CalorieCalculatorActivity extends AppCompatActivity {
    ...
    private static final String K_GENDER_POS = "gender_pos"; // Simpan posisi spinner
    ...
    private Spinner spinnerGender, spinnerActivity, spinnerGoal; // spinnerGender ditambahkan
    ...
    private void initViews() {
        ...
        spinnerGender = findViewById(R.id.spinnerGender); // Inisialisasi spinner baru
    ...
    private void saveData(int age, double height, double weight) {
        ...
        pref.edit()
                .putInt(K_GENDER_POS, spinnerGender.getSelectedItemPosition())
                .putInt(K_AGE, age)
                .putFloat(K_HEIGHT, (float) height)
                .putFloat(K_WEIGHT, (float) weight)
                .putInt(K_ACTIVITY_POS, spinnerActivity.getSelectedItemPosition())
                .putInt(K_GOAL_POS, spinnerGoal.getSelectedItemPosition())
                .apply();
    }

    private void loadSavedData() {
        SharedPreferences pref = getSharedPreferences(PREF_NAME, MODE_PRIVATE);
        spinnerGender.setSelection(pref.getInt(K_GENDER_POS, 0));
        int age = pref.getInt(K_AGE, 0);
        float h = pref.getFloat(K_HEIGHT, 0);
        float w = pref.getFloat(K_WEIGHT, 0);

        if (age > 0) etAge.setText(String.valueOf(age));
        if (h > 0) etHeight.setText(String.valueOf(h));
        if (w > 0) etWeight.setText(String.valueOf(w));

        spinnerActivity.setSelection(pref.getInt(K_ACTIVITY_POS, 0));
        spinnerGoal.setSelection(pref.getInt(K_GOAL_POS, 0));
    }
